...
export default async function handler(req, res) {
  ...
  const booking = await prisma.booking.findUnique({ where: { id: bookingId }, include: { room: { include: { listing: true }}, user: true } });
  if (!booking) return res.status(404).json({ error: 'Booking not found' });
  if (booking.userId !== user.userId) return res.status(403).json({ error: 'Forbidden' });
      ...
      line_items: [
        {
          price_data: {
            currency,
            product_data: { name: `${booking.room.title} at ${booking.room.listing.title}` },
            unit_amount: amount
          },
          quantity: 1
        }
